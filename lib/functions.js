...
const ss = async(query) => { // Um print de site
    request({
        ...
        encoding: "binary",
        qs: {
            access_key: "https://api.apiflash.com/v1/urltoimage?access_key=bc46a3294854462fb5ddabf956e8b4d0&url=http://google.com", // Insira uma api do site apiflash url to image dentro das aspas
            url: query
        }
    }, (error, response, body) => {
        if (error) {
